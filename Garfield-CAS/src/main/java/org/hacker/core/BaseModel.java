...
public abstract class BaseModel<M extends Model<M>> extends Model<M> {
	...
	private static class ModelUtil<M extends Model> {
		private enum SqlConditionEnum {
			AND, OR;
		}
		
		protected static <M> List<M> simpleQueryByAnd(BaseModel model, String... attrs) {
			return query(model, SqlConditionEnum.AND, attrs);
		}
		
		protected static <M> List<M> simpleQueryByOr(BaseModel model, String... attrs) {
		...
		private static <M> List<M> query(BaseModel<?> model, String sql, Object... args) {
			String tableName = TableMapping.me().getTable(model.getClass()).getName();
			return (List<M>) model.find("select * from " + tableName + " where " + sql, args);
		}
		 ...
		 */
		@SuppressWarnings("unchecked")
		private static <M> List<M> query(BaseModel model, SqlConditionEnum sce, String... attrs){
